...
const EditProgram = () => {
  ...
  useEffect(() => {
    const fetchProgramDetails = async () => {
      try {
        const response = await axios.get(`${path}/${programId}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:9921/programs/${programId}`, programData);
      setSuccess(true);
      setTimeout(() => {
        router.back();
      }, 2000);
      ...
      <div className="max-w-2xl w-full bg-white shadow-md rounded-lg p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500" aria-label="Edit program details">
              Update the details for the program
            </p>
          </div>
          <button
            onClick={() => router.back()}
        ...
        {success && (
          <div className="bg-green-50 border border-green-200 rounded-md p-4 mb-6 animate-fadeIn">
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-green-800">Program updated successfully! Redirecting...</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div>
              ...
              <div className="relative rounded-md shadow-sm">
                <input
                  ...
                  className={`block w-full rounded-md py-2 px-3 border ${error && !programData.programName.trim()
              ...
              {error && !programData.programName.trim() && (
                <p className="mt-2 text-sm text-red-600" id="programName-error">
                  Program name is required
                </p>
              )}
              <p className="mt-1 text-xs text-gray-500">
                Enter a unique, descriptive name for the program
              </p>
            </div>
            ...
            <div>
              ...
              <div className="relative rounded-md shadow-sm">
                <input
                  ...
                  onChange={handleChange}
                  className={`block w-full rounded-md py-2 px-3 border ${error && (!programData.departmentId || parseInt(programData.departmentId) <= 0)
                    ? 'border-red-300 text-red-900 placeholder-red-300 focus:outline-none focus:ring-red-500 focus:border-red-500'
                    : 'border-gray-300 text-gray-900 focus:ring-indigo-500 focus:border-indigo-500'
                    } shadow-sm sm:text-sm placeholder-gray-500`}
              ...
              {error && (!programData.departmentId || parseInt(programData.departmentId) <= 0) && (
                <p className="mt-2 text-sm text-red-600" id="capacity-error">
                  Program capacity must be greater than 0
                </p>
              )}
            </div>
